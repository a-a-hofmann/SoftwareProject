...
# Flow timeouts
FLOW_IDLE_TIMEOUT = 0#10
FLOW_HARD_TIMEOUT = 0#30
...
class PathInstalled (Event):
  ...
  def __init__ (self, path, src, dst):
    self.path = path
    self.src_dpid, self.src_port = src
    self.dst_dpid, self.dst_port = dst
...
class Switch (EventMixin):
  ...
  def _handle_PacketIn (self, event):
	...
	#print str(EthAddr(packet.src)), str(EthAddr(packet.dst))
